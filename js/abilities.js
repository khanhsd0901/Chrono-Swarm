...
class StasisField extends ChronoAbility {
  constructor() {
    super(
      "Stasis Field",
      GameConstants.STASIS_FIELD_COOLDOWN,
      GameConstants.STASIS_FIELD_COST,
      1
    );
    this.fields = [];
  }
  ...
  activate(player, targetPosition) {
    ...
    const fieldPosition = Vector2.add(
      playerCenter,
      Vector2.multiply(normalizedDirection, distance)
    );
...
class StasisFieldEffect extends Entity {
  ...
  render(ctx, camera) {
    ...
    // *** FIX: Add a check for finite values to prevent rendering crashes ***
    if (
      ...
      return; // Don't render if the values are invalid
    }
    ...
    gradient.addColorStop(
      0,
      `rgba(0, 255, 255, ${alpha * 0.2 * pulseIntensity})`
    );
    gradient.addColorStop(
      0.7,
      `rgba(0, 255, 255, ${alpha * 0.4 * pulseIntensity})`
    );
...
class Echo extends ChronoAbility {
  constructor() {
    super(
      "Echo",
      GameConstants.ECHO_COOLDOWN,
      GameConstants.ECHO_COST,
      GameConstants.UNLOCK_LEVELS.ECHO
    );
    this.echoes = [];
  }
...
class EchoEffect extends Entity {
  ...
  update(deltaTime) {
    ...
    if (this.isPlayingBack && this.movementHistory.length > 0) {
      ...
      const historyIndex = Math.floor(
        (playbackTime / this.recordingTime) * this.movementHistory.length
      );
  ...
  render(ctx, camera) {
    if (
      !this.isPlayingBack ||
      this.playbackIndex >= this.movementHistory.length
    )
      return;
...
class Rewind extends ChronoAbility {
  constructor() {
    super(
      "Rewind",
      GameConstants.REWIND_COOLDOWN,
      GameConstants.REWIND_COST,
      GameConstants.UNLOCK_LEVELS.REWIND
    );
  }
