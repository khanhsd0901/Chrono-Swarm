    ...
    allPlayers.forEach((player) => {
      if (player.cells.length > 0) {
        ...
        const radius = Math.max(
          2,
          Math.min(8, Math.sqrt(player.getTotalMass()) / 10)
        );

        minimapCtx.fillStyle =
          player === this.player ? "#ffffff" : player.color;
      ...
      if (
        ...
        do {
          position = new Vector2(
            MathUtils.random(
              chunk.worldX + 50,
              chunk.worldX + GameConstants.CHUNK_SIZE - 50
            ),
            MathUtils.random(
              chunk.worldY + 50,
              chunk.worldY + GameConstants.CHUNK_SIZE - 50
            )
          );
          attempts++;
