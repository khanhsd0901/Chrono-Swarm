...
class Player {
  ...
  updatePinwheelFormation(center, deltaTime) {
    this.cells.forEach((cell, index) => {
      const angle =
        (index / this.cells.length) * Math.PI * 2 + Date.now() * 0.001;
      const radius = 50;
      ...
      cell.velocity = Vector2.add(
        cell.velocity,
        Vector2.multiply(direction, 0.02)
      );
    });
  }
  ...
  updateGridFormation(center, deltaTime) {
    ...
    this.cells.forEach((cell, index) => {
      ...
      cell.velocity = Vector2.add(
        cell.velocity,
        Vector2.multiply(direction, 0.03)
      );
    });
  }
  ...
  split(direction) {
    ...
    for (const cell of this.cells) {
      if (
        ...
        cell.mass >= GameConstants.MIN_SPLIT_MASS * 2
      ) {
        largestCell = cell;
  ...
  getCenterPosition() {
    ...
    this.cells.forEach((cell) => {
      weightedPos = Vector2.add(
        weightedPos,
        Vector2.multiply(cell.position, cell.mass)
      );
      totalMass += cell.mass;
    });
...
class AIPlayer extends Player {
  ...
  makeStrategicDecisions(threats, prey, food, currentZone) {
    ...
    if (prey.length > 0 && this.aggressiveness > 0.6) {
      ...
      const targetVelocity =
        target.cells.length > 0 ? target.cells[0].velocity : new Vector2(0, 0);
      const predictedPosition = Vector2.add(
        target.getCenterPosition(),
        Vector2.multiply(targetVelocity, 2)
      );
  ...
  handleStrategicPositioning(threats, prey, food, rifts) {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const distance = 200;
      const testPos = Vector2.add(
        myPosition,
        Vector2.fromAngle(angle, distance)
      );

      // Check if position is safe from threats
      let safetyScore = 1000;
      threats.forEach((threat) => {
        const distToThreat = Vector2.distance(
          testPos,
          threat.player.getCenterPosition()
        );
  ...
  executeHumanLikeMovement(deltaTime, threats, prey) {
    ...
    if (
      this.getTotalMass() > 300 &&
      Math.random() < 0.02 &&
      this.riskTaking > 0.5
    ) {
      // Occasionally eject mass to move faster or as bait
      const ejectDirection = Vector2.subtract(
        this.target,
    ...
    if (threats.length > 0) {
      ...
      if (nearestThreat.distance < 100) {
        ...
        movementModifier =
          0.2 + this.riskTaking * 0.3 + this.tacticalAwareness * 0.2;

        // Smart splitting when being chased
        if (
          this.tacticalAwareness > 0.7 &&
          this.getTotalMass() > 200 &&
          Math.random() < 0.1
        ) {
          this.splitToEscape();
        }
    ...
    if (this.predictiveSkill > 0.6 && prey.length > 0) {
      ...
      if (targetPlayer.cells.length > 0) {
        ...
        const predictedTarget = Vector2.add(
          this.target,
          Vector2.multiply(targetVelocity, predictionTime)
        );
    ...
    if (Math.random() < 0.02) {
      ...
      const currentDirection = Vector2.subtract(
        this.target,
        this.getCenterPosition()
      );
  ...
  considerAbilityUsage(threats, prey) {
    ...
    if (this.abilityTiming > 0.7 && window.abilityManager) {
      ...
      if (
        threats.length > 0 &&
        threats[0].distance < 120 &&
        this.getTotalMass() > 150
      ) {
        const stasisField = window.abilityManager.abilities.stasis;
        if (stasisField && stasisField.canUse(this)) {
          window.abilityManager.useAbility(
            "stasis",
            this,
            threats[0].player.getCenterPosition()
          );
  ...
  findNearbyFood(chronoMatter) {
    ...
    for (const matter of chronoMatter) {
      ...
      if (distance < 300) {
        // Look for food within 300 units
        food.push({
          matter: matter,
          distance: distance,
          priority: 1 / distance,
  ...
  findAllies(players) {
    ...
    for (const player of players) {
      ...
      const theirMass = player.getTotalMass();
      const massRatio =
        Math.abs(myMass - theirMass) / Math.max(myMass, theirMass);
  ...
  coordinateWithAllies(allies, threats, prey) {
    ...
    if (strongAllies.length > 0) {
      ...
      if (humanPlayer && !humanPlayer.isAI && humanPlayer.isAlive) {
        const distanceToHuman = Vector2.distance(
          this.getCenterPosition(),
          humanPlayer.getCenterPosition()
        );
      ...
      const bigThreats = threats.filter(
        (threat) => threat.player.getTotalMass() > this.getTotalMass() * 2
      );
  ...
  handleWandering(threats, prey, food, rifts) {
    ...
    if (prey.length > 0) {
      const baseAggressiveness =
        this.aggressiveness + (this.frenzyAggressionBoost || 0);
      const shouldHunt =
        this.getTotalMass() > 80 * baseAggressiveness &&
        (this.riskTaking > 0.4 ||
          this.getTotalMass() > prey[0].player.getTotalMass() * 1.5);
    ...
    if (food.length > 0) {
      ...
      const shouldFeed =
        this.patience > 0.3 || prey.length === 0 || this.getTotalMass() < 200;
    ...
    if (
      this.stateTimer > wanderInterval ||
      !this.target ||
      timeSinceLastTarget > 3000
    ) {
  ...
  handleHunting(threats, prey, food, rifts) {
    ...
    if (prey.length > 0) {
      ...
      const shouldSplit =
        distance < 100 + this.aggressiveness * 50 &&
  ...
  handleFleeing(threats, prey, food, rifts) {
    if (threats.length > 0) {
      ...
      if (this.cells.length < 3 && this.getTotalMass() > 200) {
        const fleeDirection = Vector2.subtract(
          this.target,
          this.getCenterPosition()
        );
  ...
  calculateFleePosition(threats) {
    ...
    for (const threat of threats) {
      const threatDirection = Vector2.subtract(
        myPosition,
        threat.player.getCenterPosition()
      );
      const normalized = Vector2.normalize(threatDirection);
      fleeDirection = Vector2.add(
        fleeDirection,
        Vector2.multiply(normalized, threat.priority)
      );
    }
    ...
    return Vector2.add(
      myPosition,
      Vector2.multiply(fleeDirection, fleeDistance)
    );
  }
  ...
  generateRandomTarget() {
    ...
    if (this.movementStyle < 0.3) {
      ...
      angle =
        this.preferredDirection + Math.sin(Date.now() * 0.001) * Math.PI * 0.5;
    } else {
  ...
  updateTerritory() {
    const now = Date.now();
    if (
      now - this.lastTerritoryUpdate <
      GameConstants.TERRITORY_UPDATE_INTERVAL
    ) {
      return;
    }
    ...
    if (this.territoryCenter) {
      ...
      const playersInTerritory = this.findPlayersInTerritory(
      ...
      if (playersInTerritory.length > 0) {
        ...
        this.territoryRadius = Math.max(
          this.territoryRadius,
          Vector2.distance(this.territoryCenter, myPosition)
        );
      }
    }
...
class GameEngine {
  ...
  initializeWorld() {
    this.worldBounds = {
      ...
      top: 0,
      right:
        typeof GameConstants !== "undefined" ? GameConstants.ARENA_WIDTH : 6000,
      bottom:
        typeof GameConstants !== "undefined"
          ? GameConstants.ARENA_HEIGHT
          : 6000,
    };
  ...
  spawnAIPlayers() {
    const aiCount =
      typeof GameConstants !== "undefined" ? GameConstants.MAX_AI_PLAYERS : 25;
    const arenaWidth = this.worldBounds.right;
    const arenaHeight = this.worldBounds.bottom;
  ...
  maintainMatterDensity() {
    ...
    if (
      ...
      console.warn(
        "WorldBounds not properly initialized, skipping matter density maintenance"
      );
      return;
    }
    ...
    const spawnRate =
      typeof GameConstants !== "undefined"
        ? GameConstants.MATTER_SPAWN_RATE
        : 2.0;

    // Spawn new matter randomly to maintain density
    if (
      this.chronoMatter.length < maxMatter &&
      Math.random() < spawnRate / 1000
    ) {
      this.spawnChronoMatter(
        Math.random() * this.worldBounds.right,
        Math.random() * this.worldBounds.bottom
  ...
  checkInteractions() {
    ...
    this.ancientArtifacts?.forEach((artifact) => {
      if (
        artifact.checkPlayerNearby &&
        artifact.checkPlayerNearby(this.player)
      ) {
    ...
    this.chronoMatter?.forEach((matter, index) => {
      if (matter && this.player) {
        ...
        const totalRadius =
          matter.radius +
          this.player.cells.reduce((total, cell) => total + cell.radius, 0) /
            this.player.cells.length;
  ...
  getRandomValidPosition(
    ...
    while (attempts < maxAttempts) {
      const position = new Vector2(
        MathUtils.random(
          GameConstants.ARENA_PADDING * 2,
          GameConstants.ARENA_WIDTH - GameConstants.ARENA_PADDING * 2
        ),
        MathUtils.random(
          GameConstants.ARENA_PADDING * 2,
          GameConstants.ARENA_HEIGHT - GameConstants.ARENA_PADDING * 2
        )
      );
      ...
      if (
        avoidPosition &&
        Vector2.distance(position, avoidPosition) < minDistanceFromAvoid
      ) {
        attempts++;
        continue;
      }
      ...
      if (
        this.chunkManager &&
        !this.chunkManager.isPositionLoaded(position.x, position.y)
      ) {
        attempts++;
        continue;
      }
  ...
  renderBackground() {
    ...
    const startX =
      Math.floor(-this.canvas.width / this.camera.zoom / 2 / gridSize) *
      gridSize;
    const startY =
      Math.floor(-this.canvas.height / this.camera.zoom / 2 / gridSize) *
      gridSize;
  ...
  renderEntities() {
    ...
    // Render all players
    const allPlayers = [this.player, ...this.aiPlayers].filter(
      (p) => p && p.isAlive
    );
    allPlayers.forEach((player) => {
      ...
      if (player.cells.length > 0) {
        ...
        this.ctx.fillText(
          player.name,
          center.x,
          center.y - player.cells[0].radius - 10
        );
      }
    });
  }
  ...
  renderMinimap() {
    ...
    const arenaWidth =
      typeof GameConstants !== "undefined" ? GameConstants.ARENA_WIDTH : 6000;
    const arenaHeight =
      typeof GameConstants !== "undefined" ? GameConstants.ARENA_HEIGHT : 6000;
      ...
      if (player && player.cells && player.cells.length > 0) {
        ...
        if (center) {
          ...
          const radius = Math.max(
            3,
            Math.min(10, Math.sqrt(player.getTotalMass()) / 8)
          );

          // Draw player with improved visibility
          minimapCtx.fillStyle =
            player === this.player ? "#ffffff" : player.color || "#ff6b6b";
    ...
    if (
      this.player &&
      this.player.cells &&
      this.player.cells.length > 0 &&
      this.camera
    ) {
      const center = this.player.getCenterPosition();
      if (center) {
        const viewWidth =
          (this.canvas.width / this.camera.zoom / arenaWidth) * minimapSize;
        const viewHeight =
          (this.canvas.height / this.camera.zoom / arenaHeight) * minimapSize;
  ...
  update(deltaTime) {
    ...
    if (this.player && this.player.isAlive) {
      ...
      if (
        this.mouseInCanvas &&
        this.mouse.worldX !== undefined &&
        this.mouse.worldY !== undefined
      ) {
        const mouseWorldPos = new Vector2(this.mouse.worldX, this.mouse.worldY);
  ...
  spawnExplorationEntities(now) {
    ...
    if (now - this.lastStormSpawn > MathUtils.random(120000, 240000)) {
      ...
      if (position) {
        const stormType = MathUtils.randomChoice([
          "LIGHTNING",
          "PLASMA",
          "VOID",
          "CRYSTAL",
  ...
  applyArtifactEffects(player, artifact) {
    ...
    if (effects.abilityRecharge) {
      player.abilityRechargeMultiplier =
        (player.abilityRechargeMultiplier || 1) * effects.abilityRecharge;
    }
    ...
    if (effects.speedBonus) {
      player.speedMultiplier =
        (player.speedMultiplier || 1) * effects.speedBonus;
    }
    if (effects.maxCellsBonus) {
      player.maxCells =
        (player.maxCells || GameConstants.MAX_CELLS) + effects.maxCellsBonus;
    }
    if (effects.massGainBonus) {
      player.massGainMultiplier =
        (player.massGainMultiplier || 1) * effects.massGainBonus;
    }
    if (effects.experienceRegen) {
      player.experienceRegenRate =
        (player.experienceRegenRate || 0) + effects.experienceRegen;
    }
  }
  ...
  spawnChronoMatter(deltaTime) {
    const baseSpawnChance =
      GameConstants.MATTER_SPAWN_RATE * (deltaTime / 1000);

    // Only spawn in loaded chunks to reduce unnecessary computation
    const loadedChunks = this.chunkManager
      ? this.chunkManager.getLoadedChunks()
      : [];

    // Zone-aware spawning with enhanced rates in special zones
    Object.entries(GameConstants.ZONES).forEach(([zoneKey, zone]) => {
      const zoneEffects = zone.effects || {}; // Ensure effects object exists
      const spawnMultiplier = zoneEffects.matterSpawnRate || 1.0;
      const adjustedSpawnChance = baseSpawnChance * spawnMultiplier;

      if (
        Math.random() < adjustedSpawnChance &&
        this.chronoMatter.length < GameConstants.MAX_MATTER_COUNT
      ) {
        // --- FIX START ---
        // If the zone has no defined boundaries, we can't spawn anything in it.
        // We'll skip this zone by returning from the forEach callback.
        if (!zone.bounds) {
          return;
        }
        // --- FIX END ---
        ...
        do {
          // Now it's safe to access zone.bounds because we checked for it above.
          position = new Vector2(
          ...
          (this.isPositionTooCloseToRifts(position, 50) ||
            (this.chunkManager &&
              !this.chunkManager.isPositionLoaded(position.x, position.y))) &&
          attempts < 20
        );
  ...
  renderArenaBoundary() {
    ...
    const bottomRight = this.camera.worldToScreen(
      new Vector2(GameConstants.ARENA_WIDTH, GameConstants.ARENA_HEIGHT)
    );
  ...
  renderExplorationEntities() {
    ...
    this.ancientArtifacts.forEach((artifact) => {
      if (
        !artifact.discovered &&
        this.camera.isInView(artifact.position, artifact.radius * 3)
      ) {
  ...
  renderDebugInfo() {
    if (this.keys.has("KeyF")) {
      // Show debug info when F is held
      const chunkInfo = this.chunkManager
        ? this.chunkManager.getChunkInfo()
        : { loaded: 0, total: 0, areas: [] };
      ...
      this.ctx.fillText(
        `Particles: ${
          window.particleSystem ? window.particleSystem.getParticleCount() : 0
        }`,
        20,
        75
      );
      ...
      this.ctx.fillText(
        `Pos: ${this.camera.position.x.toFixed(
          0
        )}, ${this.camera.position.y.toFixed(0)}`,
        20,
        105
      );
      this.ctx.fillText(`Chunks: ${chunkInfo.loadedCount}`, 20, 120);
      this.ctx.fillText(
        `Player Chunk: ${chunkInfo.playerChunk.x},${chunkInfo.playerChunk.y}`,
        20,
        135
      );
      ...
      if (this.player) {
        this.ctx.fillText(
          `Mass: ${this.player.getTotalMass().toFixed(0)}`,
          20,
          150
        );
      }
  ...
  handleDynamicEvents(deltaTime) {
    ...
    if (
      now - this.lastEventTime > this.eventCooldown &&
      this.activeEvents.length < 2
    ) {
  ...
  startEvent(eventType) {
    ...
    const eventNotifications = {
      chrono_storm: {
        ...
        message:
          "Temporal energy surges through the arena! Matter spawns rapidly!",
        type: "warning",
      },
      mass_surge: {
    ...
    switch (eventType) {
      ...
      case "gravity_shift":
        ...
        event.centerPoint = new Vector2(
          MathUtils.random(
            GameConstants.ARENA_WIDTH * 0.2,
            GameConstants.ARENA_WIDTH * 0.8
          ),
          MathUtils.random(
            GameConstants.ARENA_HEIGHT * 0.2,
            GameConstants.ARENA_HEIGHT * 0.8
          )
        );
      ...
      case "temporal_freeze":
        ...
        event.affectedPlayers = this.aiPlayers.filter(
          () => Math.random() < 0.6
        );
      ...
      case "magnetic_field":
        ...
        event.magneticCenter = new Vector2(
          MathUtils.random(
            GameConstants.ARENA_WIDTH * 0.3,
            GameConstants.ARENA_WIDTH * 0.7
          ),
          MathUtils.random(
            GameConstants.ARENA_HEIGHT * 0.3,
            GameConstants.ARENA_HEIGHT * 0.7
          )
        );
  ...
  createChronoStorm(event) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      this.temporalRifts.push(
        new TemporalRift(position.x, position.y, MathUtils.random(30, 60))
      );
    }
  ...
  updateGravityShift(event, deltaTime) {
    ...
    allPlayers.forEach((player) => {
      player.cells.forEach((cell) => {
        ...
        if (distance > 10) {
          const force =
            (event.gravityStrength / (distance * distance)) * cell.mass;
          const acceleration = Vector2.multiply(
          ...
          cell.velocity = Vector2.add(
            cell.velocity,
            Vector2.multiply(acceleration, deltaTime / 1000)
          );
        }
      });
    });
  ...
  updateShrinkingArena(event, deltaTime) {
    ...
    const currentWidth = MathUtils.lerp(
      event.originalWidth,
      event.targetWidth,
      progress
    );
    const currentHeight = MathUtils.lerp(
      event.originalHeight,
      event.targetHeight,
      progress
    );
    ...
    allPlayers.forEach((player) => {
      player.cells.forEach((cell) => {
        ...
        if (
          Math.abs(cell.position.x - centerX) > maxDistX ||
  ...
  updateMagneticField(event, deltaTime) {
    ...
    allPlayers.forEach((player) => {
      player.cells.forEach((cell) => {
        ...
        if (distance > 10) {
          const force =
            (event.magneticStrength / (distance * distance)) * cell.mass;
          const acceleration = Vector2.multiply(
            Vector2.normalize(direction),
            force
          );
    ...
    allPlayers.forEach((player) => {
      ...
      if (currentZone) {
        ...
        if (currentZone.zone.effects.damageMultiplier) {
          player.zoneDamageMultiplier =
            currentZone.zone.effects.damageMultiplier;
        }
  ...
  renderZones(ctx) {
    // Render zone boundaries and effects
    Object.entries(GameConstants.ZONES).forEach(([zoneKey, zone]) => {
      if (!zone.bounds) return; // Skip if no bounds defined
      const bounds = zone.bounds;
  ...
  cleanupUnusedResources() {
    ...
    if (this.chronoMatter.length > GameConstants.MAX_MATTER_COUNT * 1.5) {
      this.chronoMatter = this.chronoMatter.slice(
        0,
        GameConstants.MAX_MATTER_COUNT
      );
    }
      ...
      if (distance > 10) {
        ...
        if (distance < phantom.radius + 30) {
          targetPlayer.cells.forEach((cell) => {
            if (
              Vector2.distance(phantom.position, cell.position) <
              phantom.radius + cell.radius
            ) {
    ...
    if (originalPlayer && originalPlayer.isAlive) {
      ...
      mirror.cells.forEach((mirrorCell, index) => {
        if (originalPlayer.cells[index]) {
          ...
          mirrorCell.position.x =
            GameConstants.ARENA_WIDTH - originalCell.position.x;
          mirrorCell.position.y =
            GameConstants.ARENA_HEIGHT - originalCell.position.y;
...
GameEngine.prototype.updateDimensionalShift = function (event, deltaTime) {
  ...
  allPlayers.forEach((player) => {
    player.cells.forEach((cell) => {
      const centerX = GameConstants.ARENA_WIDTH / 2;
      const centerY = GameConstants.ARENA_HEIGHT / 2;
      const relativePos = Vector2.subtract(
        cell.position,
      ...
      const rotatedPos = Vector2.fromAngle(
        Math.atan2(relativePos.y, relativePos.x) +
          (event.rotationSpeed * deltaTime) / 1000,
    ...
    player.cells.forEach((cell) => {
      event.timeFluctuations.forEach((fluctuation) => {
        ...
        if (distance < fluctuation.radius) {
          ...
          cell.velocity = Vector2.multiply(
            cell.velocity,
      ...
      player1.cells.forEach((cell1, index) => {
        if (player2.cells[index]) {
          ...
          const avgVelocity = Vector2.multiply(
            Vector2.add(cell1.velocity, cell2.velocity),
            0.5
          );
...
GameEngine.prototype.updateChronoPlague = function (event, deltaTime) {
  ...
  event.infectedPlayers.forEach((infected) => {
    ...
    allPlayers.forEach((player) => {
      ...
      const distance = Vector2.distance(
        infected.getCenterPosition(),
        player.getCenterPosition()
      );
      if (
        distance < event.plagueRadius &&
        Math.random() < event.infectionChance
      ) {
...
GameEngine.prototype.createMassFusion = function (event) {
  ...
  for (let i = 0; i < 5; i++) {
    const fusionZone = {
      position: new Vector2(
        MathUtils.random(
          GameConstants.ARENA_WIDTH * 0.1,
          GameConstants.ARENA_WIDTH * 0.9
        ),
        MathUtils.random(
          GameConstants.ARENA_HEIGHT * 0.1,
          GameConstants.ARENA_HEIGHT * 0.9
        )
      ),
      radius: 100,
      active: true,
...
GameEngine.prototype.createTemporalParadox = function (event) {
  ...
  for (let i = 0; i < 6; i++) {
    const anomaly = {
      position: new Vector2(
        MathUtils.random(
          GameConstants.ARENA_WIDTH * 0.15,
          GameConstants.ARENA_WIDTH * 0.85
        ),
        MathUtils.random(
          GameConstants.ARENA_HEIGHT * 0.15,
          GameConstants.ARENA_HEIGHT * 0.85
        )
      ),
  ...
  for (let i = 0; i < allPlayers.length; i += groupSize) {
    ...
    if (group.length >= 2) {
      event.entanglementGroups.push({
        ...
        sharedAbilities: MathUtils.randomChoice([
          "speed",
          "mass",
          "vision",
          "stealth",
    ...
    if (duplicate.originalPlayer && duplicate.originalPlayer.isAlive) {
      ...
      duplicate.cells.forEach((duplicateCell, index) => {
        if (duplicate.originalPlayer.cells[index]) {
          ...
          duplicateCell.position.x =
            originalCell.position.x + MathUtils.random(-50, 50);
          duplicateCell.position.y =
            originalCell.position.y + MathUtils.random(-50, 50);
        }
      });
    }
  });
    ...
    allPlayers.forEach((player) => {
      player.cells.forEach((cell) => {
        ...
        if (distance < vortex.radius) {
          ...
          const force =
            ((vortex.radius - distance) / vortex.radius) * vortex.strength;
          const normalizedDir = Vector2.normalize(direction);

          cell.position = Vector2.add(
            cell.position,
    ...
    allPlayers.forEach((player) => {
      player.cells.forEach((cell) => {
        ...
        if (distance < explosion.radius) {
          ...
          const force =
            ((explosion.radius - distance) / explosion.radius) *
            explosion.force;
          const normalizedDir = Vector2.normalize(direction);
    ...
    if (playersInZone.length >= 2) {
      for (let i = 0; i < playersInZone.length - 1; i++) {
        for (let j = i + 1; j < playersInZone.length; j++) {
          ...
          const distance = Vector2.distance(
            player1.getCenterPosition(),
            player2.getCenterPosition()
          );
          if (
            distance < 50 &&
            player1.cells.length + player2.cells.length <=
              GameConstants.MAX_CELLS
          ) {
            // Perform fusion
...
GameEngine.prototype.updateQuantumEntanglementNetwork = function (
  event,
  deltaTime
) {
          ...
          switch (pattern.type) {
            case "cyclone":
              ...
              const angle = Math.atan2(
                cell.position.y - pattern.position.y,
                cell.position.x - pattern.position.x
              );
              const newAngle = angle + (pattern.intensity * deltaTime) / 1000;
              const radius = distance;
              cell.position.x =
                pattern.position.x + Math.cos(newAngle) * radius;
              cell.position.y =
                pattern.position.y + Math.sin(newAngle) * radius;
              break;
